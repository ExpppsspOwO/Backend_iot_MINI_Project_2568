...
const temp = "exisprov2/sensor/temperature";
const hue = "exisprov2/sensor/humidity";
const light = "network_Owo/sw1";
...
client.on("connect", () => {
  ...
  client.subscribe([temp, hue, light], (err) => {
    if (!err) {
...
io.on('connection', (socket) => {
  ...
  socket.on('update_light', (data) => {
    ...
    client.publish('network_Owo/sw1', String(data)); // ส่ง MQTT กลับไป
  });
...
// app.get("/", (req, res) => {
//   cons[rows] = db.query("SELECT * FROM users", (err, rows) => {
//     if (err) {
...
//     const rows = await db.query("SELECT * FROM users where email = ? AND password = ?", [req.body.email, req.body.password])
//     console.log("ข้อมูลที่ดึงมา:", rows[0]);
